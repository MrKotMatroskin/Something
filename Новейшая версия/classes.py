...
class Wall():
    def __init__(self, A, B):
        # A, B - концы стены, определяем ориентацию вектора
        if A[0] != B[0]:
            if A[0] < B[0]:
                self.A = A
                self.B = B
        ...
        self.b = B[1] - self.k*B[0]
    def reflection(self, obj):
        # проекции луча
        xobj = obj.start_pos[0] - obj.end_point[0]
        yobj = obj.start_pos[1] - obj.end_point[1]
        if self.k != 0:
            if obj.start_pos[1] > self.k * obj.start_pos[0] + self.b:
                if (self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5) > 0:
                    self.angle_reflect = obj.angle + 2 * np.arccos(np.abs((self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5)))
                elif (self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5) < 0:
                    self.angle_reflect = obj.angle - 2 * np.arccos(np.abs((self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5)))
            elif obj.start_pos[1] < self.k * obj.start_pos[0] + self.b:
                if (self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5) > 0:
                    self.angle_reflect = obj.angle - 2 * np.arccos(np.abs((self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5)))
                elif (self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5) < 0:
                    self.angle_reflect = obj.angle + 2 * np.arccos(np.abs((self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5)))
        if (self.x * xobj + self.y * yobj) / (((self.x ** 2 + self.y ** 2) * (xobj ** 2 + yobj ** 2)) ** 0.5) == 0:
            self.angle_reflect = obj.angle + np.pi
